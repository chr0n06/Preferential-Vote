...
BEGIN
 eVoteUtil.getRatioOfInFavor(1);
 eVoteUtil.getNumberOfInFavor(2);
 eVoteUtil.getNumberOfParticipantPoll(1);
 answer:= eVoteUtil.electorRegtToVote(1,6);
 dbms_output.put_line(answer);
 answer:= eVoteUtil.subjectRegtToVote(1,1);
 dbms_output.put_line(answer);
 eVoteUtil.createDecision(21,1,1,1);
 eVoteUtil.loneScan(1);
 eVoteUtil.polyscan(1);
END;


/*PACKAGE eVoteUtil*/

create or replace PACKAGE eVoteUtil AS

PROCEDURE getRatioOfInFavor (PollSubjectID DECISION.firstchoice%TYPE);

PROCEDURE getNumberOfInFavor (PollSubjectID DECISION.firstchoice%TYPE);

PROCEDURE getNumberOfParticipantPoll (PollID POLL.ID%TYPE);

FUNCTION electorRegtToVote (PollID Poll.ID%TYPE, ELECTORID POLL.ID%TYPE) 
RETURN POLL.ID%TYPE;

FUNCTION subjectRegtToVote (PollID Poll.ID%TYPE, Pollsubj POLL.ID%TYPE)
RETURN POLL.ID%TYPE;

PROCEDURE createDecision (PollID POLL.ID%TYPE, ElectorID POLL.ID%TYPE, DecisionID POLL.ID%TYPE, PollSubjectID POLL.ID%TYPE);

PROCEDURE loneScan(PollID Poll.id%TYPE);

PROCEDURE polyscan(PollID Poll.id%TYPE);

END eVoteUtil;


/*PACKAGE BODY eVoteUtil*/

create or replace PACKAGE BODY eVoteUtil AS
